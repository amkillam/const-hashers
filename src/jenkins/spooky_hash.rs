...
/// state indices: 0..11
#[inline(always)]
const fn mix(data: &[u64], state: &mut [u64; mem::size_of::<u64>() * 3]) {
...
#[inline]
const fn end_partial(state: &mut [Wrapping<u64>; 12]) {
    state[11] = state[11].wrapping_add( state[1]);
    state[2] ^= state[11];
...
#[inline]
const fn end(data: &[u64; mem::size_of::<u64>() * 3], state: &mut [u64; mem::size_of::<u64>() * 3]) {
    state[0] = state[0].wrapping_add( data[0]);
    state[1] = state[1].wrapping_add( data[1]);
    state[2] = state[2].wrapping_add( data[2]);
    state[3] = state[3].wrapping_add( data[3]);
...
const fn short(message: &[u8], length: usize, hash1: &mut u64, hash2: &mut u64) {
    ...
    while chunk.len() >= 4 * mem::size_of::<u64>() {
        ...
        chunk = const_slice_window(chunk, 4 * mem::size_of::<u64>(), chunk.len() -  4 * mem::size_of::<u64>());
    }

    if chunk.len() >= 2 * mem::size_of::<u64>() {
                    
            let words: &[u64] = 
            if (mem::align_of_val(message) & 3) == 0 {
        ...
        for chunk in bytes[processed..].chunks(SC_BLOCK_SIZE) {
            if chunk.len() == SC_BLOCK_SIZE {
                ...
                if offset_to_align(&chunk, 8) == 0 {
                    ...
                    let mut data: &[u64] =
                        unsafe { mem::transmute::<&[u8], &[u64]>(&self.m_data) };
                    mix(&data, &mut self.m_state);
                }
            } else {
